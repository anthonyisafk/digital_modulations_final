'''
Blind modulation classification using two Deep Neural Networks.
***********************************************************
...
for which we calculate the C61, C80 cumulants.
...
def calculate_main_cumulants():
	...
	with open("test.csv") as csv_file:
		...
		for row in csv_reader:
			...
			calculated_cumulants = [c42, c21, c41, c20, c40, c65]
...
'''
def train_secondary_dnn():
	data = calculate_secondary_cumulants()

	# DNN: input layer 2 nodes,
	# 2 hidden layers 10 and 30 nodes,
	...
	with open("test2.csv") as csv_file:
		...
		for row in csv_reader:
			...
			if len(cumulant_array) > 0:
				...
				'''Catalogue of some of the other metrics we tried out.'''
				#c61 = kstat(cumulant_array, (0, 0, 0, 0, 0, 1))
				#c60 = kstat(cumulant_array, (0, 0, 0, 0, 0, 0))
				#c66 = kstat(cumulant_array, (1, 1, 1, 1, 1, 1))
				#c65 = kstat(cumulant_array, (0, 1, 1, 1, 1, 1))
			...
			calculated_cumulants = [c42, c21, c41, c20, c40]
...
def test_secondary_dnn(model):
	...
	for i in range(len(x_test)):
		if y_test[i] == ynew[i]:
			correct += 1
		print(i + 239, "X=%s, Y=%s, Predicted=%s, Prob=%s" % (x_test[i], mods[y_test[i]], mods[ynew[i]], yprob[i]))
...
'''
Tests the results of the combination of the two networks.
'''
def test_combination(primary_model, secondary_model):
	...
	# test2.csv is the file containing all the samples whose
	# modulations are to be predicted.
	with open("test2.csv") as csv_file:
		csv_reader = csv.reader(csv_file, delimiter=',')
		line_count = 0
		samples = [[]]
	...
	# Give the cumulants of the QPSK and 8-PSK samples to the secondary DNN
	# and keep its predictions.
	ynew1 = secondary_model.predict_classes(x_test1)
	yprob1 = secondary_model.predict_proba(x_test1)
	...
	for i in range(len(ynew)):
		if ynew[i] == 2 or ynew[i] == 1:
			if max(yprob[i]) < 0.6:
				...
				qpsk_score = 0.45 * yprob[i][1] + 0.55 * yprob1[i - 480][0]
				epsk_score = 0.55 * yprob[i][2] + 0.45 * yprob1[i - 480][1]
...
#convert_i_to_j('test2.csv')
# Start with training the main network, or load an existing one.
#main_model = train_main_dnn()
#main_model.save('modulation_classification(original).model')
#main_model = models.load_model('modulation_classification(original).model')
#test_main_dnn(main_model)

# Train or load the secondary network.
#secondary_model = train_secondary_dnn()
#secondary_model.save('modulation_classification_second(c88).model')
#secondary_model = models.load_model('modulation_classification_second(c88 c65).model')
#test_secondary_dnn(secondary_model)


primary_model = models.load_model('modulation_classification(original).model')
secondary_model = models.load_model('modulation_classification_second(c88 c65).model')
